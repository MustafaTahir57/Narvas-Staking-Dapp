  ...
  const getValue = async () => {
    try {
      if (connectWallets == "Connect Wallet") {
        ...
        let userInfo = await contract.methods.userInfo(2, connectWallets).call()
        userInfo = Number(userInfo.amount) /1e18
        stMyStakedValue(userInfo)

        let lockups = await contract.methods.lockups(2).call()
        ...
        let pendingReward = await contract.methods.pendingReward(connectWallets, 2).call()
        ...
        if( userStakeCount>=1){
          let rewardDebtArray = [];
            for (var i = 0; i < userStakeCount; i++) {
                ...
                if (Number(userStakes.stakeType) == 2) {
    ...
    try {
      if (connectWallets == "Connect Wallet") {
        ...
        if(withdrawAmount <= amount){
          ...
          let withdraw = await contract.methods.withdraw(amounts, 2)
          .send({
            from: connectWallets,
    ...
    try {
      if (connectWallets == "Connect Wallet") {
        ...
        if(depositAmount <=parseInt(balanceOf)){
          ...
          let deposit = await contract.methods.deposit(amount, 2)
            .send({
              from: connectWallets,
  ...
  const handleClaimReward = async () => {
    try {
      if (connectWallets == "Connect Wallet") {
        ...
        let contract = integrateContract()
        let performanceFee = await contract.methods.performanceFee().call()
        let claimReward = await contract.methods.claimReward(2)
        .send({
           from :  connectWallets,
           value: Number(performanceFee)
        })
  ...
  const handleCompoundReward = async () => {
    try {
      if (connectWallets == "Connect Wallet") {
        ...
        let contract = integrateContract();
        let performanceFee = await contract.methods.performanceFee().call()
        let compoundReward = await contract.methods.compoundReward(2).send({
          from: connectWallets,
          value: Number(performanceFee)
        })
        if(compoundReward){
      ...
      <div className="staking__blocks">
        <div className="staking__pool">
          <div className="pool__subheader-cont">
            ...
            <h4 className="pool__subheader">90 Days</h4>
          </div>
