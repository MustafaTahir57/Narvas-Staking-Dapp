...
const AboutFour = ({ connectWallets }) => {
  ...
  const getValue = async () => {
    try {
      if (connectWallets == "Connect Wallet") {
        ...
        let lockups = await contract.methods.lockups(3).call()
        ...
        let pendingReward = await contract.methods.pendingReward(connectWallets, 3).call()
       ...
       const percentage = (Number(blockNumber) / bonusEndBlock) * 100;
        ...
        let userInfo = await contract.methods.userInfo(3, connectWallets).call()
        ...
        if( userStakeCount>=1){
          let rewardDebtArray = [];
            for (var i = 0; i < userStakeCount; i++) {
                ...
                if (Number(userStakes.stakeType) == 3) {
    ...
    try {
      if (connectWallets == "Connect Wallet") {
        ...
        if(withdrawAmount <= amount){
          ...
          let withdraw = await contract.methods.withdraw(amounts, 3)
          .send({
            from: connectWallets,
    ...
    try {
      if (connectWallets == "Connect Wallet") {
        ...
        if(depositAmount <=parseInt(balanceOf)){
          ...
          let deposit = await contract.methods.deposit(amount, 3)
            .send({
              from: connectWallets,
  ...
  const handleClaimReward = async () => {
    try {
      if (connectWallets == "Connect Wallet") {
        ...
        let contract = integrateContract()
        let performanceFee = await contract.methods.performanceFee().call()
        let claimReward = await contract.methods.claimReward(3)
        .send({
           from :  connectWallets,
           value: Number(performanceFee)
        })
  ...
  const handleCompoundReward = async () => {
    try {
      if (connectWallets == "Connect Wallet") {
        ...
        let contract = integrateContract();
        let performanceFee = await contract.methods.performanceFee().call()
        let compoundReward = await contract.methods.compoundReward(3).send({
          from: connectWallets,
          value: Number(performanceFee)
        })
        if(compoundReward){
      ...
      <div className="staking__blocks">
        <div className="staking__pool">
          <div className="pool__subheader-cont">
            ...
            <h4 className="pool__subheader">180 Days</h4>
          </div>
